...
const TRUSTPILOT_URL = "https://www.trustpilot.com/evaluate/leanimg.com";

function TrustpilotIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="currentColor" aria-hidden="true">
      <path d="M12 1.5l2.76 8.5h8.94l-7.23 5.26 2.76 8.5L12 18.5l-7.23 5.26 2.76-8.5L.3 10H9.24z" />
    </svg>
  );
}
  ...
  const links = [
    {
      ...
      href: TRUSTPILOT_URL,
      icon: TrustpilotIcon,
      buildHref: () => TRUSTPILOT_URL,
    },
    {
      label: t("shareX"),
      icon: XIcon,
      buildHref: (url: string) =>
        `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(t("shareText"))}`,
    },
    {
