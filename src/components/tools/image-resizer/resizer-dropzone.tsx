...
const MAX_FILES = 10;
const MAX_TOTAL_BYTES = 50 * 1024 * 1024; // 50 MB (resize is lighter than compression)
...
export function ResizerDropzone({ onFiles, disabled }: ResizerDropzoneProps) {
  ...
  const validateFiles = useCallback(
    (files: File[]): string | null => {
      if (files.length > MAX_FILES) {
        return tErrors("tooManyFiles", { count: files.length, max: MAX_FILES });
      }
      const totalSize = files.reduce((sum, f) => sum + f.size, 0);
      if (totalSize > MAX_TOTAL_BYTES) {
        return tErrors("totalTooLarge", {
  ...
  const handleDrop = useCallback(
    ...
    [onFiles, disabled, showError, validateFiles],
  );
  ...
  const handleChange = useCallback(
    ...
    [onFiles, showError, validateFiles],
  );
