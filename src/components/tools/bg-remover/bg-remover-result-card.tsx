...
interface BgRemoverResultCardProps {
  entry: BgRemovalFileEntry;
...
function ComparisonSlider({
  ...
  return (
    <figure
      ...
      style={{
        backgroundImage:
          "repeating-conic-gradient(#d1d5db 0% 25%, #f3f4f6 0% 50%)",
        backgroundSize: "16px 16px",
      }}
    >
      {/* Bottom layer: result (transparent on checkerboard) */}
      ...
      {/* Top layer: original, clipped from left */}
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src={originalSrc}
        alt={`${alt} — original`}
  ...
  return (
    <div className="mt-2 space-y-1">
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>{t("panel.estimatedTime")}</span>
      </div>
    </div>
  );
}

export function BgRemoverResultCard({ entry, onRemove }: BgRemoverResultCardProps) {
  const t = useTranslations("bgRemover");
      ...
      <div className="rounded-lg border bg-card">
        ...
        <div className="flex items-center justify-between gap-4 px-4 py-3">
          ...
          <div className="flex items-center gap-1">
            <Button
              size="sm"
              onClick={() => downloadBlob(result.blob, outputFileName, "remove-background")}
            >
