...
export function IconDropzone({ onFile, disabled }: IconDropzoneProps) {
  ...
  const handleFile = useCallback(
    (file: File) => {
      if (!ACCEPTED_TYPES.includes(file.type)) {
        showError(tErrors("unsupportedFormat"));
        return;
      }
    ...
    >
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-sm text-muted-foreground">
          {t("dropzone.hint", { maxSize: formatFileSize(MAX_FILE_BYTES) })}
        </p>
      </div>
      {error && (
